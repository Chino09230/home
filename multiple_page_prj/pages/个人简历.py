...
with c1:
    ...
    learn = st.selectbox('国家：',['火之国', '水之国', '雷之国','风之国','土之国'], format_func=my_format_func, index=2)
    languages = st.multiselect('查克拉属性：',['火', '风', '雷','土','水','阴'],format_func=my_format_func)
    job = st.text_input('应聘职位', autocomplete='job')
    tl = st.text_input('特殊身份', autocomplete='tl')
    email = st.text_input('忍术', autocomplete='email') 
    age = st.number_input("年龄", min_value=1, max_value=130, value=10)
    date = st.date_input("生日", value=None)
    w3 = st.time_input("每日最佳联系时间段", datetime(2019, 7, 6, 21, 15))
...
with c2:
    ...
    with a3:
        ...
        st.write("**每日最佳联系时间段**:", w3)
        s = ''
        for k in languages:
            s = s  + k + '/'
        st.write('**查克拉属性**:', s)
        st.write("**在此行业工作**: ", age, '年')
